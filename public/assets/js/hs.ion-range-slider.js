...
(function ($) {
	...
	$.HSCore.components.HSIonRangeSlider = {
		...
		init: function (el, options) {
			if (
				!el.length ||
				el.attr("data-hs-ion-range-slider-options") === undefined
			)
				return;

			var context = this,
				defaults = Object.assign({}, context.defaults),
				dataSettings = el.attr("data-hs-ion-range-slider-options")
					? JSON.parse(el.attr("data-hs-ion-range-slider-options"))
					: {},
				settings = {
					onStart: function (data) {
						...
						if (settings.result_min_target_el && settings.type === "single") {
							...
							settings.result_min_target_el ||
							(settings.result_max_target_el && settings.type === "double")
						) {
							if ($(settings.result_min_target_el).is("input")) {
								$(settings.result_min_target_el).val(data.from);
							} else {
						...
						if (settings.secondary_target_el) {
							...
							settings.secondary_val.values.push(
								settings.secondary_val.values[
									settings.secondary_val.values.length - 1
								] + 1
							);

							for (var i = 0; i < settings.secondary_val.steps.length; i++) {
								if (
									data.from >= settings.secondary_val.steps[i] &&
									data.from < settings.secondary_val.steps[i + 1]
								) {
									if ($(settings.secondary_target_el).is("input")) {
										$(settings.secondary_target_el).val(
											settings.secondary_val.values[i]
										);
									} else {
										$(settings.secondary_target_el).text(
											settings.secondary_val.values[i]
										);
									}
								}
					...
					onChange: function (data) {
						...
						if (
							options &&
							options.cusOnChange &&
							typeof options.cusOnChange === "function"
						) {
							options.cusOnChange();
						}
					},
			...
			if (
				...
				$(settings.result_min_target_el).is("input")
			) {
				...
				settings.result_min_target_el ||
				(settings.result_max_target_el &&
					settings.type === "double" &&
					$(settings.result_min_target_el).is("input")) ||
				$(settings.result_max_target_el).is("input")
			) {
				$(settings.result_min_target_el).on("change", function () {
					newIonRangeSliderInstance.update({
						from: $(this).val(),
