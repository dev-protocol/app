	...
	$.HSCore.components.HSSelect2 = {
		...
		init: function (el, options) {
			...
			var context = this,
				...
				dataSettings = el.attr("data-hs-select2-options")
					? JSON.parse(el.attr("data-hs-select2-options"))
					: {},
				settings = {
		...
		multiplePlaceholder: function (newSelect2, params) {
			...
			let selection = $(newSelect2).next(".select2").find(".select2-selection"),
				...
				).append(settings.multiplePlaceholder);
		...
		formatData: function (params) {
			...
			result = settings.element.dataset.optionTemplate
				? settings.element.dataset.optionTemplate
