	...
	$.HSCore.components.HSCountdown = {
		...
		init: function (el, options) {
			...
			var context = this,
				...
				dataSettings = el.attr("data-hs-countdown-options")
					? JSON.parse(el.attr("data-hs-countdown-options"))
					: {},
				settings = {
					endDate: dataSettings.endDate
						? new Date(dataSettings.endDate)
						: new Date(),
					startDate: dataSettings.startDate
						? new Date(dataSettings.startDate)
						: new Date(),
				};
		...
		_initPiesImplementation: function (el, params) {
			...
			if (settings.yearsEl.length) {
				context._preparePieItem(settings.yearsEl, {
					maxValue:
						settings.endDate.getFullYear() - settings.startDate.getFullYear(),
			...
			if (settings.daysEl.length) {
				context._preparePieItem(settings.daysEl, {
					maxValue: context._getDaysMaxValByFormat(
						settings.daysFormat,
						settings.startDate,
						settings.endDate
					),
			...
			return el.countdown(settings.endDate, function (e) {
				...
				if (settings.yearsEl.length) {
					settings.yearsEl
						.data("circle")
						.update(e.strftime(settings.yearsFormat));
				}
				...
				if (settings.monthsEl.length) {
					settings.monthsEl
						.data("circle")
						.update(e.strftime(settings.monthsFormat));
				}
				...
				if (settings.daysEl.length) {
					settings.daysEl
						.data("circle")
						.update(e.strftime(settings.daysFormat));
				}
				...
				if (settings.hoursEl.length) {
					settings.hoursEl
						.data("circle")
						.update(e.strftime(settings.hoursFormat));
				}
				...
				if (settings.minutesEl.length) {
					settings.minutesEl
						.data("circle")
						.update(e.strftime(settings.minutesFormat));
				}
				...
				if (settings.secondsEl.length) {
					settings.secondsEl
						.data("circle")
						.update(e.strftime(settings.secondsFormat));
				}
			});
		},
