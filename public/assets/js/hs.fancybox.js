	...
	$.HSCore.components.HSFancyBox = {
		...
		init: function (el, options) {
			...
			var context = this,
				...
				dataSettings = $el.attr("data-hs-fancybox-options")
					? JSON.parse($el.attr("data-hs-fancybox-options"))
					: {},
				settings = {
					beforeShow: function (instance) {
						var $fancyOverlay = $(instance.$refs.bg[0]),
							$fancySlide = $(instance.current.$slide),
							dataSettings = instance.current.opts.$orig[0].dataset
								.hsFancyboxOptions
								? JSON.parse(
										instance.current.opts.$orig[0].dataset.hsFancyboxOptions
								  )
								: {},
							transitionEffectCustom = dataSettings.transitionEffectCustom
								? dataSettings.transitionEffectCustom
								: false,
					...
					afterShow: function (instance) {
						var $fancySlide = $(instance.current.$slide),
							$fancyPrevSlide = $(instance.group[instance.prevPos].$slide)
								? $(instance.group[instance.prevPos].$slide)
								: null,
							dataSettings = instance.current.opts.$orig[0].dataset
								.hsFancyboxOptions
								? JSON.parse(
					...
					beforeClose: function (instance) {
						...
						if (transitionEffectCustom) {
							$fancySlide
								.removeClass(transitionEffectCustom.onShow)
								.addClass(transitionEffectCustom.onHide);
