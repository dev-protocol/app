	...
	$.HSCore.components.HSClipboard = {
		...
		init: function (el, options) {
			...
			var context = this,
				...
				dataSettings = $el.attr("data-hs-clipboard-options")
					? JSON.parse($el.attr("data-hs-clipboard-options"))
					: {},
				settings = {
					shortcodes: {},
					...
					title: $el.attr("title"),
					container: !!dataSettings.container
						? document.querySelector(dataSettings.container)
						: false,
					text: function (button) {
						var dataSettings = JSON.parse(
							$(button).attr("data-hs-clipboard-options")
						);
			...
			newClipboard.on("success", function () {
				...
				if (settings.successText) {
					if (settings.type === "tooltip") {
						$el
							.attr("data-original-title", settings.successText)
							.tooltip("show");
		...
		setShortcodes: function (el, params) {
			...
			if ($(settings.contentTarget).is("input, textarea, select")) {
				settings.shortcodes[settings.contentTarget] = $(
					settings.contentTarget
				).val();
			} else {
				settings.shortcodes[settings.contentTarget] = $(
					settings.contentTarget
