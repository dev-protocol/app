...
(function (jQuery) {
	...
	function init() {
		...
		for (
			var localeGroupIdx = 0;
			localeGroupIdx < nfAllLocales.length;
			localeGroupIdx++
		) {
	...
	jQuery._formatNumber = function (
		...
		if (
			...
			(options.autoDetectPercentage && suffix.charAt(suffix.length - 1) == "%")
		) {
			number = number * 100;
		}
		...
		if (options.format.indexOf(".") > -1) {
			...
			var decimalValue = new Number(
				number.toString().substring(number.toString().indexOf("."))
			);
		...
		var onePortion = "";
		if (
			!(ones == 0 && onesFormat.substr(onesFormat.length - 1) == "#") ||
			forcedToZero
		) {
