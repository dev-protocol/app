	...
	$.HSCore.components.HSSlickCarousel = {
		...
		init: function (el, options) {
			...
			var context = this,
				...
				dataSettings = el.attr("data-hs-slick-carousel-options")
					? JSON.parse(el.attr("data-hs-slick-carousel-options"))
					: {},
				settings = {
					id: el.attr("id"),
				};
			...
			if (
				$(settings.asNavFor)[0] &&
				$(settings.asNavFor)[0].dataset.hsSlickCarouselOptions
					? JSON.parse($(settings.asNavFor)[0].dataset.hsSlickCarouselOptions)
							.isThumbsProgress
					: false
			) {
		...
		transformOff: function (el, params, event, slick) {
			...
			if (
				settings.isThumbs &&
				settings.slidesToShow >= $(slick.$slides).length
			) {
			...
			$(animatedElements).each(function () {
				var animationIn = $(this).data("hs-slick-carousel-animation"),
					...
					animationDuration = $(this).data(
						"hs-slick-carousel-animation-duration"
					);
		...
		setCustomLineDots: function (el, params) {
			...
			$dotsItems.each(function () {
				$(this).append(
					'<span class="dot-line"><span class="dot-line-helper" style="transition-duration: ' +
						(params.autoplaySpeed + params.speed) +
						'ms;"></span></span>'
				);
			});
		},
		...
		setCustomProgressCircle: function (el, params, event, slick) {
			...
			$(slick.$slides).each(function (i) {
				...
				$(slick.$slides[i])
					.children(settings.thumbsProgressContainer)
					.append($el);
			});
			...
			$style.text(
				".slick-thumb-progress .slick-thumb-progress__path {" +
					...
					"fill: transparent;" +
					"stroke: " +
					settings.thumbsProgressOptions.color +
					";" +
					"stroke-width: " +
					settings.thumbsProgressOptions.width +
					";" +
					"stroke-dashoffset: " +
		...
		setCounter: function (params, event, slick) {
			...
			$(settings.counterSelector).html(
				'<span class="' +
					settings.counterClassMap.current +
					'">1</span><span class="' +
					settings.counterClassMap.divider +
					'">' +
					settings.counterDivider +
					'</span><span class="' +
					settings.counterClassMap.total +
					'">' +
					slick.slideCount +
					"</span>"
			);
		...
		setCustomClasses: function (el, event, slider, currentSlide, nextSlide) {
			var nxtSlide = $(slider.$slides)[nextSlide],
				...
				animatedElements = $(nxtSlide).find(
					"[data-hs-slick-carousel-animation]"
				),
		...
		animationOut: function (params, event, slick, currentSlide) {
			...
			$(slick.$slides[currentSlide]).addClass(
				"animated " + settings.animationOut
			);
		},
		...
		setPosition: function (params, event, slick) {
			...
			$(slick.$slides[slick.currentSlide]).addClass(
				"animated set-position " + settings.animationIn
			);
		},
