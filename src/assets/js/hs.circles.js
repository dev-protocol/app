	...
	$.HSCore.components.HSCircles = {
		...
		init: function (el, options) {
			...
			var context = this,
				...
				dataSettings = el.attr("data-hs-circles-options")
					? JSON.parse(el.attr("data-hs-circles-options"))
					: {},
				settings = {
						...
						if (dataSettings.type === "iconic") {
							...
							if (dataSettings.additionalTextType === "prefix") {
								if (dataSettings.secondaryText) {
									return (
										(dataSettings.additionalText || "") +
										(dataSettings.isHideValue ? "" : value) +
										'<div style="margin-top: ' +
										(dataSettings.dividerSpace / 2 + "px" || "0") +
										"; margin-bottom: " +
										(dataSettings.dividerSpace / 2 + "px" || "0") +
										';"></div>' +
										'<div style="font-weight: ' +
										dataSettings.secondaryTextFontWeight +
										"; font-size: " +
										dataSettings.secondaryTextFontSize +
										"px; color: " +
										dataSettings.secondaryTextColor +
										';">' +
										dataSettings.secondaryText +
										"</div>"
									);
								} else {
									return (
		...
		setTextStyles: function (el, initEl, params) {
			...
			el.find(
				'[class="' + (settings.textClass || initEl._textClass) + '"]'
			).css({
		...
		setStrokeLineCap: function (el, initEl, params) {
			...
			el.find('[class="' + initEl._valClass + '"]').attr(
				"stroke-linecap",
				settings.fgStrokeLinecap
			);
		},
		...
		initAppear: function (initEl, params) {
			...
			appear({
				...
				appear: function (el) {
					initEl.update(
						JSON.parse($(el).attr("data-hs-circles-options")).value
					);
				},
			});
		},
