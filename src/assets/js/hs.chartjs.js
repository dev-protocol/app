	...
	$.HSCore.components.HSChartJS = {
		...
		init: function (el, options) {
			...
			var context = this,
				...
				dataSettings = el.attr("data-chartjs-options")
					? JSON.parse(el.attr("data-chartjs-options"))
					: {},
				settings = {};
			settings = $.extend(
				true,
							...
							custom: function (tooltipModel) {
								...
								if (tooltipModel.body) {
									...
									bodyLines.forEach(function (body, i) {
										var oldBody = body[0],
											newBody =
												oldBody.substring(0, oldBody.length - 3) +
												(settings.type !== "doughnut" ? "," : "") +
												oldBody.substring(oldBody.length - 3);

										innerHtml +=
											(settings.options.tooltips.hasIndicator
												? '<span class="d-inline-block rounded-circle mr-1" style="width: 10px; height: 10px; background-color: ' +
